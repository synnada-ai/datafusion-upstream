...
fn replace_order_by_clause(order_by: &mut String) {
    ...
    if let Some(start) = order_by.find("ORDER BY [") {
        if let Some(end) = order_by[start..].find(']') {
            ...
            let column_order_details = &order_by[order_by_start..=order_by_end];
            println!("column_order_details: {:?}", column_order_details);
            for &(suffix, replacement) in &suffixes {
                if column_order_details.ends_with(suffix) {
                    let new_order_details =
                        column_order_details.replace(suffix, replacement);
                    order_by
                        .replace_range(order_by_start..=order_by_end, &new_order_details);
                    break;
                }
            }
        }
